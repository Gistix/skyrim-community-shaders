...
void DX12SwapChain::RecreateWrappedResources(const DXGI_SWAP_CHAIN_DESC1& desc)
{
	...
	auto newSwapChainBuffer = std::make_unique<WrappedResource>(texDesc11, globals::dx12Interop->d3d11Device.get(), globals::dx12Interop->d3d12Device.get());

	// UI buffer uses R8G8B8A8_UNORM - vanilla UI is SDR and 8-bit precision
	texDesc11.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	auto newUiBuffer = std::make_unique<WrappedResource>(texDesc11, globals::dx12Interop->d3d11Device.get(), globals::dx12Interop->d3d12Device.get());
