...
void Streamline::LoadInterposer()
{
	...
	auto& rtLoaded = globals::features::raytracing.loaded;
	...
	pref.renderAPI = rtLoaded ? sl::RenderAPI::eD3D12 : sl::RenderAPI::eD3D11;

	logger::info("Render API: {}", magic_enum::enum_name(pref.renderAPI));
...
bool Streamline::FeatureAvailable()
{
	return loadedFeatures != Features::kNone;
}
...
void Streamline::SetDLSSDOptions(sl::ViewportHandle p_viewport, uint32_t width)
{
	...
	dlssOptions.ultraQualityPreset = sl::DLSSDPreset::ePresetE;
...
void Streamline::EvaluateDLSS(ID3D12GraphicsCommandList4* commandList, sl::ViewportHandle vp,
	ID3D12Resource* colorIn, ID3D12Resource* colorOut, ID3D12Resource* depth, ID3D12Resource* mvec,
	const sl::Extent& extentIn, const sl::Extent& extentOut, uint32_t outputWidth)
{
...
void Streamline::Upscale(ID3D12GraphicsCommandList4* a_commandList, ID3D12Resource* a_input, ID3D12Resource* a_output, ID3D12Resource* a_depth, ID3D12Resource* a_motionVectors)
{
	...
	EvaluateDLSS(a_commandList, viewport,
		a_input, a_output,
		a_depth, a_motionVectors,
		extentIn, extentOut, (uint)screenSize.x);
}

void Streamline::DenoiseUpscale(ID3D12GraphicsCommandList4* a_commandList, ID3D12Resource* a_upscalingTexture, ID3D12Resource* a_depth, ID3D12Resource* a_motionVectors)
{
